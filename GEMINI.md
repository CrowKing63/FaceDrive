# MagicScroll Gemini Assistant Context

This document provides context for the Gemini AI assistant to understand and assist with the MagicScroll project.

## Project Overview

MagicScroll is a macOS application that allows users to scroll using facial expressions. It captures video from the camera, uses the Vision framework to detect facial landmarks, and translates expressions like mouth open and eye close into scroll actions.
...
*   `MagicScrollApp.swift`: The main entry point of the application. It sets up the app delegate and the main scene.
...
*   `DashboardView.swift`: The main control panel, where users can configure sensitivity and actions for facial expressions.
*   `FaceTracking/`: This directory contains the code related to face detection and tracking.
    ...
    *   `ExpressionDetector.swift`: Calculates expression values from facial landmarks.
...
*   `Input/`: This directory contains the code related to input control.
    *   `InputController.swift`: Uses CoreGraphics to programmatically control scrolling.
    *   `ActionMapper.swift`: Maps facial expressions to scroll actions.
    *   `FaceModels.swift`: Defines the data structures for facial expressions and actions.

## Building and Running

To build and run the project, open `MagicScroll.xcodeproj` in Xcode and click the "Run" button.

**Important:** The application requires Accessibility permissions to control scrolling. If the application doesn't work, make sure to grant Accessibility permissions in System Settings > Privacy & Security > Accessibility.
